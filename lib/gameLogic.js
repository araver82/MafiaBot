...
game.getPossibleRolesets = (length) =>{
    return _.filter(game.getRolesets(), set => set.roles.length == length);
};
...
game.pickRoleSet = (possibleRolesets) => {
    ...
    for (var i = 0; i < game.variations.length; i++) {
        if (game.variations.canMutate(roleset, game.variations[i]) && Math.random() < 0.25) {
            roleset = game.variations.mutate(roleset, game.variations[i]);
        }
    }
    return roleset;
};
